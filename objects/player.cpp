...
std::vector<PlayerConfig> loadPlayerConfigs(const std::string &filePath) {
  ...
  if (file.is_open()) {
    while (std::getline(file, line)) {
      ...
      if (std::getline(ss, value, ','))
        config.mass = std::stoi(value);
      if (std::getline(ss, value, ','))
        config.accelerator_factor = std::stof(value);
      if (std::getline(ss, value, ','))
        config.maxVel = std::stof(value);
...
void Player::render() {
  if (texture) {
    ...
    SDL_Color color =
        (team == 1) ? SDL_Color{0, 0, 255, 255} : SDL_Color{255, 0, 0, 255};
...
void Player::move(int stateMove, int fieldX, int fieldY, int fieldWidth,
                  int fieldHeight) {
  if (stateMove == SDL_SCANCODE_S || stateMove == SDL_SCANCODE_DOWN) {
    ...
    this->velocity_x *= 0.85f; // Reduce X velocity by 15% (slower deceleration)
    this->velocity_y *= 0.85f; // Reduce Y velocity by 15% (slower deceleration)

    // Stop completely if velocity is very close to zero to prevent tiny,
    // perpetual movement
...
bool Player::loadPlayer(const char *texturePath) {
  ...
  if (surface) {
    ...
    if (texture) {
      ...
      std::cout << "Failed to create player texture: " << SDL_GetError()
                << std::endl;
      return false;
    }
  } else {
